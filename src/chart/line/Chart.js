...
import CAxisInfoBase_In from '../../base/info/CAxisInfoBase_In.js';
import CAxisInfoBase_Out from '../../base/info/CAxisInfoBase_Out.js';

import VAxisInfoBase_In from '../../base/info/VAxisInfoBase_In.js';
import VAxisInfoBase_Out from '../../base/info/VAxisInfoBase_Out.js';

import MarkPoint from './parts/MarkPoint.js';
...
export default class Chart extends ChartBase {
    ...
    constructor(parentDivID) {
        ...
        this.cAxisInfo_In = new CAxisInfoBase_In();           //类目轴信息
        ...
        this._cAxisInfo_Out = new CAxisInfoBase_Out();    //数据结构体(存储计算结果，供内部用)
        this._vAxisInfo_Out = new VAxisInfoBase_Out();

        this._itemArray = [];         //存放'标记点数据'的二维数组   
        this._itemPool = null;        //'标记点数据'对象池

        this._chartPixelWidth;        //图表有效尺寸
        this._chartPixelHeight;

        this._parentDivClassName = 'lineChartDiv';    //样式名
        this._tipDivClassName = 'lineChartTipDiv';
        ...
        //初始化对象池
        this._itemPool = new ObjectPool(MarkPoint);
    ...
    _reset () {
        ...
        this._tweenPositionWidth = this._chartPixelWidth + 4;
    ...
    _addOrRemoveEventListener (command) {
        ...
        function doMouseMove (e) {
            ...
            //根据位置计算其代表的类目索引
            index = Math.round((position.x - left) / categorySize);
